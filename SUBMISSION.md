...
> **代码版本**：[`23ae899`](https://github.com/cagedbird043/brilliant-sort/commit/23ae899832c1085f058356aacd41d34f0e2b2ae0)（2026-07-16）  
> **线上 Demo**：[Hong Kong](https://brilliant-sort.cagedbird.cn/) · [GitHub Pages](https://cagedbird043.github.io/brilliant-sort/)  
> **CI 证据**：[Actions #29492121811](https://github.com/cagedbird043/brilliant-sort/actions/runs/29492121811)（`verify`、`publish-hk`、`deploy` 全部成功）
...
题目要求的是 Demo 的核心逻辑与工程结构；仓库额外实现了可在线游玩的 Tux 旗舰关卡、确定性像素资产管线、独立 C++ AudioWorklet 合成器、双站静态部署、两级本地 Git 门禁，以及四种 Agent 可直接使用的 OpenSpec 技能。这些额外部分不改变核心论点：**规则唯一、边界可驱动、状态可序列化、失败可定位、AI 输出可验证。**

## 1. 玩法假设（8 行）

以下 8 条统一按本 Demo 的建模假设处理，不声称还原视频未展示的原产品规则。题目明确给出的八方向连通首先是代码题契约；将它复用于玩法选择，以及容量、排序、胜利和魔法棒规则，仍属于可替换的 Demo 实现选择。
...
这些假设被版本化，是为了让 Demo、Harness 和测试有同一套可执行口径，不是把隐藏规则猜成事实。后续若拿到正式规则，可以替换对应规格和夹具；随机生成、商业 Power-up、LiveOps、账号状态等未展示能力继续留在待确认/延期清单。
...
重玩功能暴露了原实现里一个很具体的生命周期错误：音频引擎把 `won` 当成“整个引擎只能提交一次”，所以第二局不会再播胜利音乐。修复没有重建 AudioContext，而是在 12 字节 Cue 协议里追加 `Restart = 7`；浏览器按 `Won → Restart → Won` 投递，C++ 引擎按局重置 Voice、Score Transport、Layer 和胜利锁存，同时保留静音状态。原生诊断断言两次 fanfare，浏览器 E2E 断言实际传入 Worklet 的 kind 序列为 `[6, 7, 6]`。
...
已归档和活动中的 OpenSpec 提供题目事实、假设、非目标、模块边界、接受标准和人工验收门。项目上下文已迁入 `openspec/config.yaml`；OpenSpec 1.6 官方生成的 `propose / apply / explore / sync / archive` 技能提交在 `.omp/`、`.codex/`、`.claude/` 和 `.opencode/`，同一套约束可以直接交给 Oh My Pi、Codex、Claude Code 与 OpenCode。`src/agent/context.ts` 会拒绝请求中包含明确延期的能力；`src/agent/audit.ts` 记录规格版本、改动文件、验证命令、结果、下一决策和时间戳。
...
Agent 的可执行反馈不是自然语言“看起来不对”，而是固定 fixture、命令日志、canonical dump、事件、rejection、Harness 首个 JSON 差异、原生/WASM 差分和浏览器 E2E。Lefthook 再把这些门禁放到真实开发路径上：pre-commit 并行跑类型、Bun 测试、夹具与严格规格，pre-push 顺序跑 native/WASM/生产构建和 desktop/mobile Playwright，避免共享构建目录互相打架。
...
作为复现成本参考，[GitHub Actions run #29492121811](https://github.com/cagedbird043/brilliant-sort/actions/runs/29492121811) 的 `verify` 在 Ubuntu runner 上用时 1 分 58 秒，其中包括安装固定版本的 Emscripten 和 Chromium、编译三个后端并运行全部自动化检查。本地已有依赖和浏览器缓存时通常更快；第一次运行主要看下载速度。

在本答卷工作站上，四项并行 pre-commit 门禁用时 1.27 秒，完整 pre-push 门禁用时 20.91 秒；这是实测值，不是对其它机器的固定承诺。
