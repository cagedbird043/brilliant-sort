...
std::string ReadSessionResult(std::uintptr_t session_handle) {
    ...
    Expect(
        bs_core_copy_result(session_handle, bytes.data(), bytes.size()) == length,
...
constexpr std::string_view kExpectedWonDump =
    "{\"schemaVersion\":1,\"levelId\":\"core-unit\",\"board\":{\"rows\":1,\"cols\":2,\"cells\":[{\"row\":0,\"col\":0,\"targetColor\":\"ice\",\"gemId\":\"b\"},{\"row\":0,\"col\":1,\"targetColor\":\"navy\",\"gemId\":\"a\"}]},\"gems\":[{\"id\":\"a\",\"color\":\"navy\"},{\"id\":\"b\",\"color\":\"ice\"}],\"shelf\":{\"width\":12,\"capacity\":12,\"gemIds\":[]},\"selection\":null,\"status\":\"won\"}";

void TestWinningTraceAndCanonicalDump() {
    std::string error;
    std::unique_ptr<BrilliantSortCore> core = BrilliantSortCore::Create(kSwapLevel, &error);
    ...
    for (const std::string_view command : trace) {
        ...
        ExpectContains(result.json, "\"canonicalDump\"", "transition should include canonical evidence");
    }

    Expect(core->CanonicalDump() == kExpectedWonDump, "winning trace should reach exact canonical dump");
    const CoreDispatchResult won_transition = core->DispatchJson(
        R"json({"type":"select-board-gem","coord":{"row":0,"col":0}})json");
    ExpectContains(won_transition.json, "\"code\":\"game-won\"", "terminal state should reject further commands");

    const CoreDispatchResult restarted = core->DispatchJson(R"json({"type":"restart-level"})json");
    Expect(!restarted.protocol_error, "restart command should parse");
    ExpectContains(restarted.json, "\"status\":\"playing\"", "restart should restore playing state");
}
    ...
    constexpr std::string_view level = R"json(
{
  ...
  "cells": [
    ...
    std::string error;
    std::unique_ptr<BrilliantSortCore> core = BrilliantSortCore::Create(level, &error);
    ...
    const CoreDispatchResult locked = core->DispatchJson(
        R"json({"type":"select-board-gem","coord":{"row":0,"col":0}})json");
    ExpectContains(locked.json, "\"code\":\"locked-gem\"", "matching gem should reject selection");
    Expect(core->CanonicalDump() == before, "rejection should not mutate state");

    const CoreDispatchResult malformed = core->DispatchJson("{");
    Expect(malformed.protocol_error, "malformed command should return protocol error");
    ExpectContains(malformed.json, "\"code\":\"invalid-command\"", "protocol error should be actionable");
}

void TestCAbiSession() {
    const std::uintptr_t session = bs_core_create(kSwapLevel.data(), kSwapLevel.size());
    Expect(session != 0U, "C ABI should create a session");
    const std::string initial = ReadSessionResult(session);
    ExpectContains(initial, "\"state\"", "C ABI creation should expose initial transition");

    constexpr std::string_view command =
        R"json({"type":"select-board-gem","coord":{"row":0,"col":0}})json";
    Expect(
        bs_core_dispatch(session, command.data(), command.size()) == 0,
        "C ABI should dispatch a valid command");
    const std::string selected = ReadSessionResult(session);
    ExpectContains(selected, "\"selection-changed\"", "C ABI should expose ordered events");
    bs_core_destroy(session);
}

void TestInvalidLevel() {
    std::string error;
    std::unique_ptr<BrilliantSortCore> core = BrilliantSortCore::Create("{}", &error);
