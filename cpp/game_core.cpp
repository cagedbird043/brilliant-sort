...
[[nodiscard]] int ChebyshevDistance(Coord left, Coord right) {
    const int row_distance = left.row >= right.row ? left.row - right.row : right.row - left.row;
    const int col_distance = left.col >= right.col ? left.col - right.col : right.col - left.col;
...
public:
    ...
    static JsonValue Object(std::vector<std::pair<std::string, JsonValue>> value) {
        JsonValue result;
...
private:
    ...
    [[nodiscard]] std::uint32_t ParseUnicodeCodeUnit() {
        ...
        for (int digit = 0; digit < 4; ++digit) {
            code_unit = (code_unit << 4U) | static_cast<std::uint32_t>(ParseHexDigit());
        }
        return code_unit;
    }
...
[[nodiscard]] const JsonValue& RequireField(const JsonValue& object, std::string_view name) {
...
[[nodiscard]] const std::string& RequireString(const JsonValue& value, std::string_view field_name) {
    if (value.kind != JsonValue::Kind::kString) {
        throw std::runtime_error("field must be a string: " + std::string(field_name));
    }
    return value.string_value;
}

[[nodiscard]] int RequireInteger(const JsonValue& value, std::string_view field_name) {
    if (value.kind != JsonValue::Kind::kNumber ||
        value.number_value < std::numeric_limits<int>::min() ||
        value.number_value > std::numeric_limits<int>::max()) {
        throw std::runtime_error("field must be a 32-bit integer: " + std::string(field_name));
    }
...
[[nodiscard]] const std::vector<JsonValue>& RequireArray(
    ...
    if (value.kind != JsonValue::Kind::kArray) {
        throw std::runtime_error("field must be an array: " + std::string(field_name));
    }
...
[[nodiscard]] std::size_t BoardIndex(const State& state, Coord coord) {
    return static_cast<std::size_t>(coord.row) * static_cast<std::size_t>(state.cols) +
           static_cast<std::size_t>(coord.col);
}

[[nodiscard]] bool IsBoardCoordInBounds(const State& state, Coord coord) {
    return coord.row >= 0 && coord.col >= 0 && coord.row < state.rows && coord.col < state.cols;
}
...
[[nodiscard]] const BoardCell* BoardCellAt(const State& state, Coord coord) {
    ...
    const std::optional<BoardCell>& cell = state.board_cells[BoardIndex(state, coord)];
    return cell ? &*cell : nullptr;
}

[[nodiscard]] const Gem* GemForId(const State& state, const std::string& gem_id) {
...
[[nodiscard]] bool IsLocked(const State& state, const BoardCell& cell) {
    return cell.gem_id.has_value() &&
           GemForId(state, *cell.gem_id) != nullptr &&
...
[[nodiscard]] std::vector<std::vector<GemCell>> BuildBoardGrid(
    const State& state,
    ...
    for (int row = 0; row < state.rows; ++row) {
        for (int col = 0; col < state.cols; ++col) {
            ...
            const Gem* gem = cell->gem_id.has_value() ? GemForId(state, *cell->gem_id) : nullptr;
...
[[nodiscard]] bool IsMovableBoardGem(const State& state, const BoardCell& cell) {
...
[[nodiscard]] std::vector<Location> FindBoardMovableComponent(const State& state, Coord start) {
    const std::vector<std::vector<GemCell>> grid = BuildBoardGrid(state, IsMovableBoardGem);
    const std::vector<Position> positions = FindConnectedMovableGems(grid, start.row, start.col);
    std::vector<Location> component;
    component.reserve(positions.size());
    for (const Position& position : positions) {
        const BoardCell* cell = BoardCellAt(state, {position.first, position.second});
...
[[nodiscard]] std::vector<Location> FindShelfMovableComponent(const State& state, int start_index) {
    if (start_index < 0 || start_index >= static_cast<int>(state.shelf_gem_ids.size())) {
        return {};
    }
    const int shelf_rows = state.shelf_capacity / kShelfWidth;
    std::vector<std::vector<GemCell>> grid(
        static_cast<std::size_t>(shelf_rows),
        std::vector<GemCell>(kShelfWidth, {-1, false}));
    for (std::size_t index = 0; index < state.shelf_gem_ids.size(); ++index) {
        const Gem* gem = GemForId(state, state.shelf_gem_ids[index]);
        if (gem == nullptr) {
            continue;
        }
        const std::size_t row = index / static_cast<std::size_t>(kShelfWidth);
        const std::size_t col = index % static_cast<std::size_t>(kShelfWidth);
        grid[row][col] = {static_cast<int>(gem->color), true};
    }

    const Coord start = {start_index / kShelfWidth, start_index % kShelfWidth};
    const std::vector<Position> positions = FindConnectedMovableGems(grid, start.row, start.col);
    std::vector<Location> component;
    component.reserve(positions.size());
    for (const Position& position : positions) {
        const std::size_t index = static_cast<std::size_t>(position.first) * kShelfWidth +
                                  static_cast<std::size_t>(position.second);
        if (index < state.shelf_gem_ids.size()) {
            component.push_back({state.shelf_gem_ids[index], {position.first, position.second}});
        }
    }
    return component;
}

[[nodiscard]] std::vector<Coord> FindEmptyTargetComponent(
    const State& state,
    Coord start,
    Color color) {
    const BoardCell* start_cell = BoardCellAt(state, start);
    if (start_cell == nullptr || start_cell->gem_id.has_value() || start_cell->target_color != color) {
        return {};
    }
    const std::vector<std::vector<GemCell>> grid = BuildBoardGrid(state, IsEmptyTarget);
    const std::vector<Position> positions = FindConnectedMovableGems(grid, start.row, start.col);
...
[[nodiscard]] std::vector<Location> GetSelectionLocations(
    const State& state,
    ...
    if (selection.container == SelectionContainer::kBoard) {
        ...
        for (std::size_t index = 0; index < state.shelf_gem_ids.size(); ++index) {
            positions_by_gem_id.emplace(
                state.shelf_gem_ids[index],
                Coord{
                    static_cast<int>(index / static_cast<std::size_t>(kShelfWidth)),
                    static_cast<int>(index % static_cast<std::size_t>(kShelfWidth)),
                });
        }
    }
...
[[nodiscard]] bool CoordinatesAreConnected(
    const State& state,
    ...
    const int rows =
        container == SelectionContainer::kBoard ? state.rows : state.shelf_capacity / kShelfWidth;
    const int cols = container == SelectionContainer::kBoard ? state.cols : kShelfWidth;
    std::vector<std::vector<GemCell>> grid(
    ...
    for (const Coord coord : coordinates) {
        ...
        grid[static_cast<std::size_t>(coord.row)][static_cast<std::size_t>(coord.col)] = {0, true};
    }
    return FindConnectedMovableGems(grid, coordinates.front().row, coordinates.front().col).size() ==
           coordinates.size();
}

[[nodiscard]] std::vector<Location> GetExtractionCandidates(
    const State& state,
    const Selection& selection) {
    const std::vector<Location> locations = GetSelectionLocations(state, selection);
    ...
    std::sort(candidates.begin(), candidates.end(), [&selection](const Location& left, const Location& right) {
        return CompareByAnchor(selection.anchor, left.coord, right.coord) < 0;
    });
    return candidates;
}
...
[[nodiscard]] bool ShelfCandidateRemainsConnectedAfterCompaction(
    ...
    const std::size_t source_index =
        static_cast<std::size_t>(candidate.coord.row) * kShelfWidth +
    ...
    compacted.erase(compacted.begin() + static_cast<std::ptrdiff_t>(source_index));
    ...
    for (const std::string& gem_id : remaining_gem_ids) {
        ...
        remaining_coordinates.push_back({
            static_cast<int>(found->second / static_cast<std::size_t>(kShelfWidth)),
            static_cast<int>(found->second % static_cast<std::size_t>(kShelfWidth)),
        });
    }
    return CoordinatesAreConnected(state, SelectionContainer::kShelf, remaining_coordinates);
}

[[nodiscard]] std::optional<Location> NextExtractionCandidate(
    const State& state,
    const Selection& selection) {
    const std::vector<Location> candidates = GetExtractionCandidates(state, selection);
    if (selection.container == SelectionContainer::kBoard) {
        if (candidates.empty()) {
            return std::nullopt;
        }
    ...
    for (const Location& candidate : candidates) {
        if (ShelfCandidateRemainsConnectedAfterCompaction(state, selection, candidate)) {
            return candidate;
        }
    }
    return std::nullopt;
...
void MoveSelectionGemToBoard(
    ...
    if (selection.container == SelectionContainer::kBoard) {
        ...
        const std::size_t source_index =
            static_cast<std::size_t>(source.coord.row) * kShelfWidth +
            static_cast<std::size_t>(source.coord.col);
        state.shelf_gem_ids.erase(
            state.shelf_gem_ids.begin() + static_cast<std::ptrdiff_t>(source_index));
    }
    RemoveSelectedGem(state, source.gem_id);
}

void MoveSelectionGemToShelf(
    State& state,
    const Selection& selection,
    const Location& source) {
    if (selection.container != SelectionContainer::kBoard) {
...
[[nodiscard]] Transition Reject(std::string_view code, std::string_view detail) {
    Transition result;
...
[[nodiscard]] Transition SelectBoardGem(State& state, Coord coord) {
    ...
    if (cell == nullptr || !cell->gem_id.has_value()) {
        return Reject("no-selectable-gem", "The Board coordinate has no movable gem");
    }
    const Gem* gem = GemForId(state, *cell->gem_id);
    if (gem == nullptr) {
        return Reject("no-selectable-gem", "The Board gem is not present in this state");
    }
    if (gem->color == cell->target_color) {
        return Reject("locked-gem", "A gem matching its Board target is locked");
    }

    const std::vector<Location> component = FindBoardMovableComponent(state, coord);
    if (component.empty()) {
        return Reject("no-selectable-gem", "The Board coordinate has no movable component");
    }
    Selection selection;
    ...
    return Transition{{{"selection-changed", std::string("board")}}, std::nullopt};
}

[[nodiscard]] Transition SelectShelfGem(State& state, int index) {
    const std::vector<Location> component = FindShelfMovableComponent(state, index);
    if (component.empty()) {
        return Reject("no-selectable-gem", "The Shelf index has no movable gem");
    }
    const Gem* gem = GemForId(state, component.front().gem_id);
    if (gem == nullptr) {
        return Reject("no-selectable-gem", "The Shelf gem is not present in this state");
    }

    Selection selection;
    selection.container = SelectionContainer::kShelf;
    selection.anchor = {index / kShelfWidth, index % kShelfWidth};
    selection.color = gem->color;
    selection.gem_ids.reserve(component.size());
    for (const Location& member : component) {
        selection.gem_ids.push_back(member.gem_id);
    ...
    state.selection = std::move(selection);
    return Transition{{{"selection-changed", std::string("shelf")}}, std::nullopt};
}
...
[[nodiscard]] Transition PlaceSelectionAtTarget(State& state, Coord coord) {
    ...
    if (target_cell == nullptr) {
        return Reject("invalid-target", "The target coordinate is not an active Board cell");
    }
    if (target_cell->gem_id.has_value()) {
        return Reject("target-is-occupied", "The target Board cell already contains a gem");
    }
    if (target_cell->target_color != state.selection->color) {
        return Reject("target-color-mismatch", "The target color does not match the selection color");
    }

    const std::vector<Coord> targets = FindEmptyTargetComponent(state, coord, state.selection->color);
    ...
    for (const Coord destination : targets) {
        ...
        const std::optional<Location> source = NextExtractionCandidate(state, current_selection);
        if (!source.has_value()) {
            break;
        }
...
[[nodiscard]] Transition PlaceSelectionInShelf(State& state) {
    if (!state.selection.has_value() || state.selection->gem_ids.empty()) {
        return Reject("no-selection", "Select a Board component before moving it to the Shelf");
    }
    ...
    const int available_slots = state.shelf_capacity - static_cast<int>(state.shelf_gem_ids.size());
...
[[nodiscard]] Coord ParseCoord(const JsonValue& object, std::string_view field_name) {
...
[[nodiscard]] Transition ApplyCommand(State& state, const State& initial_state, const Command& command) {
    if (command.type == CommandType::kRestartLevel) {
        state = initial_state;
        return {};
    }
    if (state.won) {
        return Reject("game-won", "Restart the level before issuing another command");
    }
    ...
    switch (command.type) {
        ...
        case CommandType::kCancelSelection:
            ...
            state.selection.reset();
            return ResolveStatus(state, Transition{{{"selection-cleared", std::nullopt}}, std::nullopt});
...
[[nodiscard]] State ParseLevel(std::string_view level_json) {
    ...
    if (RequireInteger(RequireField(root, "schemaVersion"), "schemaVersion") != kSchemaVersion) {
    ...
    state.shelf_capacity = RequireInteger(RequireField(root, "shelfCapacity"), "shelfCapacity");
    if (state.rows <= 0 || state.cols <= 0) {
        throw std::runtime_error("Level dimensions must be positive");
    }
    if (state.shelf_capacity <= 0 || state.shelf_capacity % kShelfWidth != 0) {
        throw std::runtime_error("Shelf capacity must be a positive multiple of 12");
    }
    if (static_cast<std::size_t>(state.rows) >
        std::numeric_limits<std::size_t>::max() / static_cast<std::size_t>(state.cols)) {
        throw std::runtime_error("Level dimensions are too large");
    }
    state.board_cells.resize(
        static_cast<std::size_t>(state.rows) * static_cast<std::size_t>(state.cols));

    std::array<int, 4> gem_color_counts{};
    std::array<int, 4> target_color_counts{};
    const std::vector<JsonValue>& cells = RequireArray(RequireField(root, "cells"), "cells");
    for (const JsonValue& cell_json : cells) {
        ...
        const std::string& gem_id = RequireString(RequireField(gem_json, "id"), "gem.id");
        const std::string& gem_color_name = RequireString(RequireField(gem_json, "color"), "gem.color");
...
[[nodiscard]] std::string StateJson(const State& state) {
    ...
    json += "},\"shelf\":{\"width\":12,\"capacity\":" +
            std::to_string(state.shelf_capacity) + ",\"gemIds\":[";
    for (std::size_t index = 0; index < state.shelf_gem_ids.size(); ++index) {
        if (index != 0U) {
            json.push_back(',');
        }
    ...
    if (!state.selection.has_value()) {
        ...
        AppendJsonString(
            json,
            selection.container == SelectionContainer::kBoard ? "board" : "shelf");
...
[[nodiscard]] std::string CanonicalStateDump(const State& state) {
    ...
    for (int row = 0; row < state.rows; ++row) {
        for (int col = 0; col < state.cols; ++col) {
            ...
            first_cell = false;
            json += "{\"row\":" + std::to_string(row) + ",\"col\":" +
                    std::to_string(col) + ",\"targetColor\":";
            AppendJsonString(json, ColorName(cell->target_color));
            json += ",\"gemId\":";
            if (cell->gem_id.has_value()) {
                AppendJsonString(json, *cell->gem_id);
...
[[nodiscard]] std::string TransitionJson(const State& state, const Transition& transition) {
    std::string json = "{\"schemaVersion\":1,\"state\":" + StateJson(state) + ",\"events\":[";
...
BrilliantSortCore::BrilliantSortCore(std::unique_ptr<Impl> impl) : impl_(std::move(impl)) {}

BrilliantSortCore::~BrilliantSortCore() = default;

BrilliantSortCore::BrilliantSortCore(BrilliantSortCore&&) noexcept = default;

BrilliantSortCore& BrilliantSortCore::operator=(BrilliantSortCore&&) noexcept = default;

std::unique_ptr<BrilliantSortCore> BrilliantSortCore::Create(
    ...
    try {
        ...
        return std::unique_ptr<BrilliantSortCore>(new BrilliantSortCore(std::move(impl)));
...
CoreDispatchResult BrilliantSortCore::DispatchJson(std::string_view command_json) {
    try {
        const Command command = ParseCommand(command_json);
        const Transition transition = ApplyCommand(impl_->state, impl_->initial_state, command);
...
std::string MakeCoreProtocolError(std::string_view code, std::string_view detail) {
...
extern "C" std::uintptr_t bs_core_create(const char* level_json_bytes, std::size_t length) {
    try {
        auto session = std::make_unique<CoreSession>();
        if (level_json_bytes == nullptr && length != 0U) {
            session->last_result = MakeCoreProtocolError(
                "invalid-level",
                "level JSON bytes are null with a non-zero length");
        } else {
            const std::string_view level_json(
                level_json_bytes == nullptr ? "" : level_json_bytes,
                length);
            std::string error_detail;
            session->core = BrilliantSortCore::Create(level_json, &error_detail);
            session->last_result = session->core != nullptr
...
extern "C" int bs_core_dispatch(
    ...
    if (session->core == nullptr) {
        session->last_result = MakeCoreProtocolError("invalid-session", "level loading did not succeed");
    ...
    const std::string_view command_json(command_json_bytes == nullptr ? "" : command_json_bytes, length);
...
extern "C" std::size_t bs_core_copy_result(
    std::uintptr_t session_handle,
    char* output_bytes,
    std::size_t capacity) {
    const CoreSession* session = SessionFromHandle(session_handle);
    if (session == nullptr || output_bytes == nullptr || capacity < session->last_result.size()) {
        return 0U;
    }
    std::memcpy(output_bytes, session->last_result.data(), session->last_result.size());
