...
The production App SHALL load `tux-01` first as a 24-column by 32-row flagship mosaic with between 450 and 550 active target sockets. Inactive coordinates SHALL remain absent around the recognizable Tux silhouette. After Tux reaches settled `Won`, the contextual action SHALL advance to the approved `chrome-01` second level; production SHALL NOT expose a level picker, random generation, placeholder levels, or persistent progression.
...
- **GIVEN** `tux-01` has reached `Won` and its authoritative victory presentation has settled
- **WHEN** the player activates the contextual next-level action
