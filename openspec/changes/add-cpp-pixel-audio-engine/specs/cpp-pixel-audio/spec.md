...
- **AND THEN** no placement, compaction, progress, or victory cue is fabricated.
...
- **AND THEN** mute control and `won` retain reserved delivery capacity.
...
- **WHEN** the engine receives the first `won` cue
- **THEN** it schedules the victory fanfare once and suppresses another normal loop section
- **AND THEN** duplicate `won` cues do not stack or restart the fanfare.
