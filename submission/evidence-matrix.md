...
| 必须覆盖的自动化场景 | 对角连通、锁定/异色屏障、Shelf 容量/压缩、拒绝、Tux 完整胜利、全局修复、引导/音频持久化、手动/魔法棒终章、contextual replay、相机复位、reduced-motion、重放确定性 | [`tests/core/`](../tests/core/)；[`tests/harness/`](../tests/harness/)；[`tests/wasm/`](../tests/wasm/)；[`tests/e2e/game.e2e.ts`](../tests/e2e/game.e2e.ts) | `bun run check`；`bun run test:e2e`；`bun run harness differential tux-01` |
