...
| Demo 一键全局修复 | `apply-global-wand` 保留锁定身份，按颜色将字典序 Gem ID 配对到行优先目标；只发一个聚合事件和一个 `won` | [活动 OpenSpec 设计](../openspec/changes/add-demo-assist-and-victory-finale/design.md)；[`src/core/types.ts`](../src/core/types.ts)；[`cpp/game_core.cpp`](../cpp/game_core.cpp) | 初始/中途状态 TypeScript、native C++、WASM canonical parity；`tests/core/global-wand.test.ts` |
