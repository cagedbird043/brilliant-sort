...
| 选择、放置、Shelf、胜利流程 | 八方向最大分量、安全边界提取、同色目标批量放置、紧凑 Shelf 压缩、`Won` 谓词 | [`src/core/topology.ts`](../src/core/topology.ts)；[`src/core/reducer.ts`](../src/core/reducer.ts)；[`cpp/game_core.cpp`](../cpp/game_core.cpp) | `tests/core/reducer.test.ts`；`cpp/game_core_test.cpp`；`tux-01.win.json` |
...
| 模拟棋盘 / 空格 / 槽位操作 | 所有 UI 操作被规范化为 `GameCommand`；Harness 直接 dispatch 相同命令 | [`src/core/types.ts`](../src/core/types.ts)；[`src/harness/cli.ts`](../src/harness/cli.ts) | `tests/e2e/game.e2e.ts`；`tests/harness/replay.test.ts` |
...
| 必须覆盖的自动化场景 | 对角连通、锁定/异色屏障、Shelf 容量/压缩、拒绝、Tux 完整胜利、重放确定性 | [`tests/core/`](../tests/core/)；[`tests/harness/`](../tests/harness/)；[`tests/wasm/`](../tests/wasm/) | `bun run check`；`bun run harness differential tux-01` |
...
| 生产表现不污染规则 | WAAPI clone、Large/Micro LOD 切换和空 Socket 透明度仅从已提交 transition 派生；不会改 state | [`src/app/App.tsx`](../src/app/App.tsx)；[`src/styles.css`](../src/styles.css) | `tests/e2e/game.e2e.ts`；最新修复 CI |
| 可复现 CI / 部署 | main 推送依次运行 typecheck、native、WASM、Bun、CTest、Chromium E2E，然后静态发布香港站与 Pages | [`.github/workflows/ci.yml`](../.github/workflows/ci.yml) | [Actions #29478518729](https://github.com/cagedbird043/brilliant-sort/actions/runs/29478518729) |

## 固定提交与复现入口

- **答卷对应提交**：[`53caf03fda09`](https://github.com/cagedbird043/brilliant-sort/commit/53caf03fda0958c4b1bff3c91b44ed5766413ac2)
